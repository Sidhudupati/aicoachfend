...
const Practice = () => {
  ...
  // Show camera preview when entering "prompt" stage
  useEffect(() => {
    if ((stage === "prompt" || stage === "recording") && !videoRef.current.srcObject) {
    ...
    // Cleanup when leaving prompt/recording
    return () => {
      if (
  ...
  // Only start recording on button click
        ...
        <>
          ...
          <div className="flex flex-col items-center">
            <div className="relative">
              <video
                ref={videoRef}
                className={`rounded-2xl shadow-2xl w-[500px] border-4 ${stage === "prompt" ? "border-blue-500" : "border-red-500"}`}
                autoPlay
                muted
              />
              {stage === "recording" && (
            ...
            {stage === "prompt" ? (
              <button
                ...
                className="mt-6 px-8 py-4 bg-green-600 text-white rounded-xl shadow-lg hover:bg-green-700 hover:shadow-xl transform hover:scale-105 transition-all font-semibold text-lg flex items-center gap-3"
              >
                <Video size={24} />
                Start Recording
              </button>
